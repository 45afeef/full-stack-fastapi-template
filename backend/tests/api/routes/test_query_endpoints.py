...
def test_query_cabs_success_and_invalid(client: TestClient, superuser_token_headers: dict[str, str], db: Session):
...
def test_query_drivers_success_and_invalid(client: TestClient, superuser_token_headers: dict[str, str], db: Session):
    username = random_email()
    password = random_lower_string()
    owner = crud.create_user(session=db, user_create=UserCreate(email=username, password=password))
...
def test_query_stay_units_near_access_and_filters(client: TestClient, superuser_token_headers: dict[str, str], normal_user_token_headers: dict[str, str], db: Session):
    ...
    # assign normal user as agency staff by creating an agency and adding staff
...
def test_sql_injection_like_input_is_handled(client: TestClient, superuser_token_headers: dict[str, str], db: Session):
